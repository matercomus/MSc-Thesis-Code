...
def filter_chinese_license_plates(
    ...
    regex_pattern: str = r"^[\u4e00-\u9fa5][A-Z][A-Z0-9]{5,6}$",
) -> pl.LazyFrame:
    """
    Efficiently filters Chinese license plates using a stricter regex pattern.
    Preserves original dtype (works with categorical/string columns).
    """
    return lazy_df.filter(pl.col(col).cast(pl.String).str.contains(regex_pattern))
